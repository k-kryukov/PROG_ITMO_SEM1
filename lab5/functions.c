...
void write_generation(byte** pixels, int h, int w, FILE* in, FILE* out, size_t offset) {
	...
	// ÍÅÎÁÕÎÄÈÌÎ ÇÀÏÈÑÀÒÜ ÍÀØ ÈÒÎÃÎÂÛÉ ÌÀÑÑÈÂ Â ÔÀÉË ÏÎÁÈÒÎÂÎ
	for (int i = h - 1; i >= 0; --i) { // êîëè÷åñòâî áèò, êîòîğûå íóæíî çàïèñàòü
		int counter = 0;
		byte accum = 0; // ×ÅĞÍÛÌ ÊĞÀÑßÒÑß ÍÓËÅÂÛÅ ÁÈÒÛ (ÇÀÊĞÀØÈÂÀŞÒÑß)
		int j = 0;
		int already_sent = 0;
		while (j < w) {
